...
const formSchema = z.object({
  ...
  items: z
    .array(
      z.object({
        title: z.string().min(1, 'Заголовок элемента обязателен'),
        description: z.string().min(1, 'Описание элемента обязательно'),
...
type OfferingsFormValues = z.infer<typeof formSchema>;

// Интерфейс для пропсов компонента
interface OfferingsFormProps {
  pageId: string;
  sectionId?: string;
  defaultValues?: OfferingsFormValues;
  onSuccess?: () => void;
}
...
export const OfferingsForm = ({
  ...
  const form = useForm<OfferingsFormValues>({
  ...
  // Инициализация массива полей для items
  ...
  const onSubmit = async (values: OfferingsFormValues) => {
    try {
      setIsLoading(true);
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <FormLabel className="text-base">Элементы</FormLabel>
            <Button
              type="button"
              variant="outline"
              size="sm"
